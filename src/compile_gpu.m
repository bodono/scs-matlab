...
cmd = sprintf(['mex -O -v %s %s %s COMPFLAGS="$COMPFLAGS %s" CFLAGS="$CFLAGS %s" ' ...
    'scs/linsys/gpu/indirect/private.c %s -Iscs -Iscs/linsys -Iscs/include ' ...
    '%s %s %s -output matlab/scs_gpu'], ...
    flags.arr, flags.LCFLAG, common_scs, flags.COMPFLAGS, flags.CFLAGS, ...
    flags.INCS, flags.link, flags.LOCS, flags.BLASLIB);
disp(cmd);
eval(cmd);
